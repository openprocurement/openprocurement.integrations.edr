...
class TestVerify(BaseWebTest):
    ...
    def test_timeout_verify(self):
        setup_routing(self.edr_api_app, func=create_long_read(0.5))
        response = self.app.get('/verify?id=1234567891')
        self.assertEqual(response.status, '200 OK')
        self.assertEqual(response.content_type, 'application/json')

        setup_routing(self.edr_api_app, func=create_long_read(3))
        response = self.app.get('/verify?id=1234567891', status=403)
        self.assertEqual(response.status, '403 Forbidden')
...
class TestDetails(BaseWebTest):
    ...
    def test_timeout_mult(self):
        ...
        setup_routing(self.edr_api_app, path='/1.0/subjects/2842335', func=create_long_read(0.2))
        ...
        setup_routing(self.edr_api_app, path='/1.0/subjects/2842335', func=create_long_read(3))
        response = self.app.get('/details/2842335', status=403)
        self.assertEqual(response.status, '403 Forbidden')
        ...
        setup_routing(self.edr_api_app, path='/1.0/subjects/2842335', func=create_long_read(5))
        response = self.app.get('/details/2842335', status=403)
        self.assertEqual(response.status, '403 Forbidden')
        self.assertEqual(response.json['errors'][0]['description'], [{u'message': u'Gateway Timeout Error'}])

        # 6.5 sec delayed response - new timeout is 7 - should succeed and set timeout to 3.5
        setup_routing(self.edr_api_app, path='/1.0/subjects/2842335', func=create_long_read(6.5))
        response = self.app.get('/details/2842335', status=200)
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(response.status, '200 OK')

        # 4 sec delayed response - new timeout is 3.5 - should fail
        setup_routing(self.edr_api_app, path='/1.0/subjects/2842335', func=create_long_read(4))
        response = self.app.get('/details/2842335', status=403)
        self.assertEqual(response.status, '403 Forbidden')
        self.assertEqual(response.json['errors'][0]['description'], [{u'message': u'Gateway Timeout Error'}])
    ...
    def test_timeout_add(self):
        ...
        setup_routing(self.edr_api_app, path='/1.0/subjects/2842335', func=create_long_read(2))
        ...
        setup_routing(self.edr_api_app, path='/1.0/subjects/2842335', func=create_long_read(6))
        response = self.app_copy.get('/details/2842335', status=403)
        self.assertEqual(response.status, '403 Forbidden')
        self.assertEqual(response.json['errors'][0]['description'], [{u'message': u'Gateway Timeout Error'}])

        # 7 sec delayed response - should fail (timeout 6 max)
        setup_routing(self.edr_api_app, path='/1.0/subjects/2842335', func=create_long_read(7))
        response = self.app_copy.get('/details/2842335', status=403)
        self.assertEqual(response.status, '403 Forbidden')
        self.assertEqual(response.json['errors'][0]['description'], [{u'message': u'Gateway Timeout Error'}])
